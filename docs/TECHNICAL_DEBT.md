...
|---|---|---|
| Root license and third-party notices | The repository has no root license grant or complete reviewed notice set. macOS maps 2,227 compiler paths to 46 ownership roots and physical iOS maps 2,578 to 41, both with zero uncovered. Every root now has an exact standalone or mechanically assembled inline-primary package notice. Secondary-notice and second-person legal-completeness review remain open. | Decide the license for DinoPad-owned work, satisfy GPL source/notice obligations, perform secondary/legal notice review, correct the corpus as needed, and audit both final products. |
| Compiled AOT rights | ROM/game files are absent from packages, but the executable embeds private AOT derived from the user-supplied game program. | Obtain an explicit rights determination or limit delivery to source-only local generation. |
