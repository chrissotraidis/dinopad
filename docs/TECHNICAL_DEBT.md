...
|---|---|---|
| Root license and third-party notices | The repository has no root license grant or complete assembled notice set. The validated macOS inventory now hashes 17 direct linked components and 8 selected resources, but full header/source transitive and iOS-product coverage remains open. | Decide the license for DinoPad-owned work, satisfy GPL source/notice obligations, complete both product inventories, assemble exact shipped notices, and audit them against the final package. |
