...
| Final privacy-manifest audit | The app now bundles and structurally audits a truthful no-tracking/no-collection manifest with required reasons for known file metadata, UserDefaults, and timer APIs. A final Xcode privacy report and transitive dependency review have not been performed. | Generate the final release privacy report, reconcile every linked SDK/API against exact pins, and retain any independently required SDK manifests before distribution. |
...
| Build-disk headroom | Available disk is 14 GiB, below the 20 GiB project gate. The complete workspace's recoverable build/scratch footprint is only about 3.7 GiB, so in-scope cleanup cannot restore the threshold. | Free or add at least 6 GiB outside the repository before another full generation/build cycle; do not delete unrelated user data autonomously. |
