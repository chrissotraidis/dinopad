...
|---|---|---|
| Root license and third-party notices | The repository has no root license grant or complete assembled notice set. The validated macOS inventory now hashes 17 direct linked components and 10 selected resources, but full header/source transitive and iOS-product coverage remains open. | Decide the license for DinoPad-owned work, satisfy GPL source/notice obligations, complete both product inventories, assemble exact shipped notices, and audit them against the final package. |
| Compiled AOT and launcher asset rights | ROM/game files are absent from packages, but the executable embeds private AOT derived from the user-supplied game program. macOS also packages DinoFont, Noto/Lato faces, logo, and character art without a complete affirmative provenance/notice record. | Obtain an explicit rights determination or limit delivery to source-only local generation; remove/replace unresolved resources or document their redistribution basis and exact required notices. |
