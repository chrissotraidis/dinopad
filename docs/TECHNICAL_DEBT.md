...
|---|---|---|
| ~~Root license and primary third-party notices~~ | **Closed 2026-08-19.** DinoPad-owned work is GPL-3.0-only with an explicit scope. macOS maps 2,187 compiler paths to 45 ownership roots and physical iOS maps 2,578 to 41, both with zero uncovered and a packaged primary notice for every root. | Keep the matching-source archive, license scope, exact pins, and generated notice corpus synchronized with each tag. Optional second-person review remains release QA. |
| Static game AOT advisory | ROM/game files are absent from packages, but the executable embeds static AOT derived from the user-supplied game program, matching upstream Dinosaur Planet: Recompiled's release architecture. | Preserve the no-ROM/no-game-assets boundary and disclose the copyright-risk advisory; do not misclassify it as a missing software license. |
| Final privacy-manifest audit | The app bundles and structurally audits a truthful no-tracking/no-collection manifest with required reasons for known file metadata, UserDefaults, and timer APIs. Goal 31m created an unsigned Organizer-input archive whose app payload passed package, compiler-inventory, and notice gates, but this host's Xcode first-launch chooser requires an uninstalled 8.52 GiB iOS platform component before Organizer can open. No aggregate report or transitive dependency review has been performed. | On a prepared Xcode host, generate the final release privacy report from a final archive, reconcile every linked SDK/API against exact pins, and retain any independently required SDK manifests before distribution. |
| DinoMod permission/license | The official GitHub and Thunderstore 0.9.3 packages declare no redistribution license. Technical static integration exists, but public Restored redistribution is not authorized. The audited base IPA excludes it and passes. | Obtain explicit maintainer permission before shipping Restored code/data; base publication is unaffected. |
| Physical-device evidence | Signed builds have run on the user's iPhone and iPad with saves/settings preserved; extended iPad Restored play and Xbox-controller reconnect are user-observed. | Complete the remaining soak/audio/thermal/update matrix as release QA. |
| ~~Build-disk headroom~~ | **Cleared on recheck 2026-08-18 (Goal 31l).** 25 GiB were free, exceeding the 20 GiB gate; a fresh unsigned physical-iOS rebuild and package audit passed. | Keep the 20 GiB preflight before each full generation/build cycle; capacity can change outside the repository. |
...
| ~~macOS bundle depended on host paths/newer objects~~ | **Closed 2026-08-18 (Goal 31e).** The app previously linked `/opt/homebrew` FreeType/libpng, copied an absolute checkout-path asset symlink, and mixed macOS 26 SDL objects into an app claiming macOS 11. FreeType 2.13.3 is now an exact push-disabled source pin built statically without optional external dependencies; pinned SDL builds in-tree at the app target with fatal linker warnings; asset copying dereferences the build symlink; and a package gate rejects non-system dylibs, symlinks, private paths, prohibited material, bad metadata, and invalid signatures. | Keep `scripts/check-macos-package-safety.sh` in every macOS bundle build and release preflight. Rights/notices and notarization remain separate P0 gates. |
| ~~Unproven macOS launcher fonts/art were packaged~~ | **Closed 2026-08-18 (Goal 31g).** DinoFont, Noto Emoji, the bitmap logo, Krazoa art, and development-only Sass are omitted. The dormant RML launcher uses text and pinned Lato, whose exact attribution and OFL text are packaged. Audit negative controls reject asset reintroduction or notice modification. | Keep the package resource allowlist, executable/resource reference checks, notice comparisons, and rights inventory synchronized. |
